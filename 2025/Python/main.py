...
days = [None] + [importlib.import_module(f"src.day{i:02d}") for i in range(1, 13)]

parser = argparse.ArgumentParser()
parser.add_argument("day", nargs="?", type=int, help="Select just one day to run")
parser.add_argument(
    "-t", "--test", action="store_true", help="Run tests instead of input"
)
parser.add_argument(
    "-d", "--debug", action="store_true", help="Include error outputs for test cases"
)
parser.add_argument(
    "-s", "--submit", action="store_true", help="Submit solutions to AoC"
)
...
def runDay(day, path=None):
    ...
    with open(path, "r") as f:
        ...
        if args.submit and results and path.endswith("input"):
            if results[0]:
                submit(results[0], part="a", day=day, year=2025)
            if results[1]:
                submit(results[1], part="b", day=day, year=2025)
...
def runTests(day):
    ...
    for test_path in test_paths:
        printDay(
            f"Test {test_path}:", *runDay(day, f"data/day{day:02d}/tests/" + test_path)
        )
        print()
...
if args.day == None:
    ...
    if args.test:
        print("(test flag ignored due to running all days)")
    print()
    total = 0
    for i in range(1, 26):
